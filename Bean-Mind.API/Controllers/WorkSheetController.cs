...
namespace Bean_Mind.API.Controllers
{
    public class WorkSheetController : BaseController<WorkSheetController>
    {
        ...
        public async Task<IActionResult> GetListWorkSheet([FromQuery] int? page, [FromQuery] int? size)
        {
            ...
            if (response == null)
            {
                return Problem(MessageConstant.WorkSheetMessage.WorkSheetIdEmpty);
            }
            return Ok(response);
        }
        ...
        [ProducesErrorResponseType(typeof(ProblemDetails))]
        public async Task<IActionResult> UpdateVideo([FromRoute] Guid id,[FromBody] UpdateWorkSheetRequest request, [FromQuery] Guid activityId, [FromQuery] Guid worksheetTemplateId)
        {
